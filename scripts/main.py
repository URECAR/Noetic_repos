...
Max_Joint_VEL   = 0.1               # Joint 이동의 속도 제한 [rad/s]
Max_Joint_ACC   = 0.1               # Joint 이동의 가속도 제한  [rad/s^2]
TABLE_EXT_HEIGHT = 500              # Object 위에 위치할 높이. [mm] 
...
posx_Machine = [112.95, 1110.23, 484.6, -3.14, 0.00, -1.61]
posx_Machine_high = posx_Machine.copy()
posx_Machine_high[2] = 526
...
class Move_Group(object):   # 로봇팔의 움직임 제어 클래스
    def __init__(self, ignore_stop=False):
        ...
        self.adjusted_pose = None   # 현재 UR posx(공간) 좌표
...
class RealSense:            # 로봇팔 말단(eef)에 부착된 Realsense D435i 제어 클래스
    def __init__(self, duration=2):
        ...
        self.ROI_POINTS = np.array([(69, 224), (565, 227), (562, 479), (64, 477)], np.int32)    # 인식 범위를 픽셀 꼭짓점으로 지정
        self.EDGE_THRESHOLD_LOW = 40
        self.EDGE_THRESHOLD_HIGH = 150
        self.MIN_CONTOUR_AREA = 5000    # 최소 인식 직사각형 넓이
        self.MIN_RECT_LENGTH = 50       # '' 길이
        self.PIXEL_TOLERANCE = 20
        self.DETECTION_THRESHOLD = 0.4  # 인식률. 
...
def PickAndGotoMid(option, num):
    ...
    if option == "inner":
        ...
        if str(num) not in posx_Objexp_list:
            print(f"Error: Object {num} not detected by camera")
            return
    ...
    print(f"Moving up to high position")
    UR.movel([posx_Obj_high], "abs")
    
    # 4단계: 테이블 high 경유점을 거쳐 Mid 위치로 이동
    print(f"Moving to Mid position via safe waypoints")
    waypoints_to_mid = [posx_Table_high, posx_Mid]
    UR.movel(waypoints_to_mid, "abs")
    print(f"Arrived at Mid position with object {num}")
...
def PlaceAtMachine():
    ...
    print(f"Moving from Mid to Machine_high")
    UR.movel([posx_Machine_high], "abs")
    
    # 2단계: Machine_high에서 Machine 위치로 하강
    print(f"Moving down to Machine position")
    UR.movel([posx_Machine], "abs")
    
    # 3단계: 그리퍼 열어서 물체 배치
    print(f"Placing object at Machine")
    gripper.open()
    UR.scene.remove_attached_object('tool0', name='box')
    
    # 4단계: Machine에서 Machine_high로 상승
    print(f"Moving up to Machine_high")
    UR.movel([posx_Machine_high], "abs")
    
    # 5단계: Machine_high에서 Mid로 복귀
    print(f"Returning to Mid position")
    UR.movel([posx_Mid], "abs")
    print(f"Arrived back at Mid position")
